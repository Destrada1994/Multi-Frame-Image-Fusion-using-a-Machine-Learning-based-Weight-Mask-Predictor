function [LR_reg, Tvec]=Register_LKO(stack_in,bfn,keep_dim)
if bfn ~=1
...
for i=2:size(stack,3)
    ...
    rforms(i)=affine2d([ D(1,1) D(2,1) 0; D(1,2) D(2,2) 0; 0 0 1]);
    tforms(i) = affine2d([ 1 0 0; 0 1 0; -D(1,3) -D(2,3) 1]);
    Tvec{i} = tforms(i);
...
for i = 1:numel(rforms)
    [xlim(i,:), ylim(i,:)] = outputLimits(rforms(i), [1 width], [1 height]);
end
...
for i=1:size(stack, 3)
    ...
    I = imwarp(squeeze(stack_in(:,:,:,i)), rforms(i), 'cubic', 'FillValues', 0,'OutputView', AlignedView);
