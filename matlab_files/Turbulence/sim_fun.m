function img_out = sim_fun(img_in,params,opts)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for C = 1:3
    ...
    %         fprintf('%3g \n', i);
            for j=1:K
                idx   = (i-1)*K+j;
                idx1  = round((i-1)*rows/K+rowsH/2+[-rowsH/2+1:rows/K+rowsH/2]);
                idx2  = round((j-1)*rows/K+rowsH/2+[-rowsH/2+1:rows/K+rowsH/2]);
        ...
        out      = num./den;
        img_blur(:,:,C,k) = out(rowsH/2+1:rows+rowsH/2, colsH/2+1:cols+colsH/2);
    end
...
for i = 1:frames
    ...
    for C=1:3
        img_out(:,:,C,i) = MotionCompensate(img_blur(:,:,C,i),MVx,MVy,0.5);
    end
end
