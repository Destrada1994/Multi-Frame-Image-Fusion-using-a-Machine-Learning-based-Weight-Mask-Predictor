...
for i in range(opt.valid_size):
    ...
    hr_img=RegisterDFT(hr_img,np.squeeze(predict))
    
    target_image=hr_img*255
    target_image=np.squeeze(target_image).astype(np.uint8)
    ...
    ssim=structural_similarity(target_image,output_image)
    PSNR=PSNR+psnr
    SSIM=SSIM+ssim
    print('PSNR = {}dB'.format(psnr))
    print('SSIM = {}dB'.format(ssim))
    f.write('image {}:Process Time={}seconds, PSNR={}dB, SSIM={}\n'.format(idx,time_process,psnr,ssim))
    ...
    output_name=join(save_dir,'SR_'+model_name+'_'+name+'_psnr'+str(psnr)+'_ssim'+str(ssim)+'.png')
...
print('Average SSIM = {}dB'.format(ssim_avg))
f.write('Average Time Process={}seconds, Average PSNR={}dB, Average SSIM={}\n'.format(avg_estimate_time,psnr_avg,ssim_avg))
f.close()